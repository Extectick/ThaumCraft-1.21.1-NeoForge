package thaumcraft.server.crafting;
...
import thaumcraft.common.lib.crafting.ArcaneWorktableRecipes.WorktableInput;
import thaumcraft.common.registry.TCRecipeTypes;

public final class ServerArcaneWorktableService {
    private ServerArcaneWorktableService() {
    }

    public static boolean tryCraft(Level level, ArcaneWorktableBlockEntity worktable, ItemStack wand, Player player) {
        WorktableInput worktableInput = ArcaneWorktableRecipes.createInput(worktable);
        Optional<RecipeHolder<ArcaneWorktableRecipe>> recipeHolder = ArcaneWorktableRecipes.findRecipe(level,
                worktableInput.input());
    ...
    public static boolean tryConsumeArcaneCraft(Level level, CraftingInput input, Container worktable, ItemStack wand,
        ...
        if (recipeHolder.isEmpty() || recipeHolder.get().value() != recipe
                || !ArcaneWorktableRecipes.hasPrimalCost(wand, player, recipe, worktableInput.input())) {
            return false;
        }
    ...
    public static boolean tryConsumeVanillaCraft(Level level, CraftingInput input, Container worktable, Player player,
        ...
        Optional<RecipeHolder<CraftingRecipe>> recipeHolder = ArcaneWorktableRecipes.findVanillaRecipe(level,
                worktableInput.input());
        if (recipeHolder.isEmpty() || recipeHolder.get().value() != recipe) {
            return false;
        }
    ...
    private static void consumePrimalCost(ItemStack wand, Player player, ArcaneWorktableRecipe recipe,
        ...
        for (Aspect aspect : Aspect.getPrimalAspects()) {
            WandVisHelper.consumeVis(wand, aspect,
                    ArcaneWorktableRecipes.effectivePrimalCost(wand, player, recipe, input, aspect));
        }
    }
