...
public final class InfusionSourceFxHandler {
    ...
    public static void add(BlockPos target, BlockPos source, int color, int ticks) {
        String key = target.getX() + ":" + target.getY() + ":" + target.getZ() + ":"
                + source.getX() + ":" + source.getY() + ":" + source.getZ() + ":" + color;
        SOURCE_FX.put(key, new SourceFx(target.immutable(), source.immutable(), ticks, color));
    }
